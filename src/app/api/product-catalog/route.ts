import { ensureDefaultProductCatalog } from "@/lib/ensure-default-catalog";
...
export async function GET() {
  ...
  if (!prisma) {
    return withCors(NextResponse.json({ stored: false, products: [] }));
  }
  try {
    await ensureDefaultProductCatalog();
    const row = await getCatalogRow(prisma.salonxProductCatalog);
    return withCors(
      NextResponse.json({
        stored: row.stored,
        products: row.items,
    ...
    console.error("[api/product-catalog GET]", e);
    return withCors(NextResponse.json({ error: "Failed to load products" }, { status: 500 }));
  }
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    console.error("[api/product-catalog PUT]", e);
